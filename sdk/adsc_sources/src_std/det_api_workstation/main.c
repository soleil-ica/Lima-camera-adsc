...
int	main(int argc, char **argv)
{
	...
	int 	ccddet_init();
	int 	mserver_init();
	static	char	*version_string = "V8.3.0";
	...
	if(NULL != (cp = getenv("CCD_DET_API_CONNECTION_LOG")))
	{
		if(NULL == (fp_connect_log = fopen(cp, "a")))
		{
		...
		else
		{
			...
			fprintf(fp_connect_log, "\n\tdet_api_workstation started at %s\n\n", char_ztime());
			fprintf(fp_connect_log, "\n=========================================================================\n");
		}
	}

	if(NULL != getenv("CCD_SEND_ALL_MODULE_STATUS"))
		ccd_send_all_module_status = 1;
	/*
	...
	sec_command_port_no = -1;
	if(NULL != (cp = (char *) getenv("CCD_DTSECPORT")))
		sec_command_port_no = atoi(cp);

	fprintf(stderr,"det_module: control_det ON using command port: %d\n",command_port_no);

	if(NULL == (cp = (char *) getenv(CCD_DET_API_LOGFILE)))
	{
	...
	if(-1 == (command_s = det_api_socket(command_port_no)))
		cleanexit(1);
	if(-1 != sec_command_port_no)
		if(-1 == (sec_command_s = det_api_socket(sec_command_port_no)))
			cleanexit(1);
