...
char	*argv[];
  {
	...
	if(argc < 7)
	{
		fprintf(stderr,"detcon_test_th bin prefix noxform start end time\n");
		exit(0);
	}
	...
	for(nimages = start; nimages <= end; nimages++)
	{
	    ...
	    CCDSetFilePar(FLP_FILENAME,buf);
	    CCDSetFilePar(FLP_KIND,&flp_kind);
	    fprintf(stdout, "FLP_FILENAME: %s and flp_kind: %d\n", buf, flp_kind);
	    CCDStartExposure();
	    while(DTC_STATE_EXPOSING != CCDState())
	      {
		if(DTC_STATE_ERROR == CCDState())
		...
		last_image = 0;
	    fprintf(stdout, "setting lastimage: %d\n", last_image);
	    CCDSetFilePar(FLP_LASTIMAGE, &last_image);
	    CCDGetImage();
	}
	fprintf(stdout,"Done.\n");
	fprintf(stdout,"Wait 5 seconds for all images to be collected by gather\n");
	system("sleep 5");
