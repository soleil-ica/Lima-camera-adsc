...
static	int				q_ncols_raw = -1;
static	int				q_nrows_raw = -1;
static	int				q_image_size = -1;
...
{
	static char	orderings[MAX_CONTROLLERS] = {'W', 'X', 'Z', 'Y'};
	int			i,v1,v2;
	...
	for(i = 0; i < MAX_CONTROLLERS; i++)
		if(qso[o1].q_assign == orderings[i])
			v1 = i;
	for(i = 0; i < MAX_CONTROLLERS; i++)
		if(qso[o2].q_assign == orderings[i])
			v2 = i;
	return(v2 - v1);
}
