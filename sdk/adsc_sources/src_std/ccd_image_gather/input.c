		...
		if(i < 0)
		  {
		    ...
		    {
			fprintf(fp_connect_log, "rep_write: at %s: Error (%d) (%s) on file descriptor %d.\n", ztime(), save_errno, strerror(save_errno), fd); 
		...
		if(i < 0)
		  {
		    ...
		    if(NULL != fp_connect_log)
		    {
			fprintf(fp_connect_log, "rep_read: at %s: Error (%d) (%s) on file descriptor %d.\n", ztime(), save_errno, strerror(save_errno), fd); 
			fflush(fp_connect_log);
		    }
		    timestamp(fplog);
	...
	if(nb == -1)
	  {
		...
		if(NULL != fp_connect_log)
		{
			fprintf(fp_connect_log, "ccd_image_gather: check_for_connection: at %s: select error.\n", ztime());
			fprintf(fp_connect_log, "ccd_image_gather: check_for_connection: Errno was %d\n\n", save_errno);
			fprintf(fp_connect_log, "ccd_image_gather: This is a fatal error causing an EXIT.\n");
			fflush(fp_connect_log);
		}
	    ...
	    if(g < 0)
	      {
			...
			if(NULL != fp_connect_log)
			{
				fprintf(fp_connect_log, "ccd_image_gather: check_for_connection: at %s: accept error.\n", ztime());
				fprintf(fp_connect_log, "ccd_image_gather: check_for_connection: Errno was %d\n\n", save_errno);
				fprintf(fp_connect_log, "ccd_image_gather: This is a fatal error causing an EXIT.\n");
				fflush(fp_connect_log);
			}
	...
	if(fd == command_fd)
	  {
		if(NULL != fp_connect_log)
		{
			fprintf(fp_connect_log, "ccd_image_gather: notify_server_eof: at %s: command connection CLOSED on client error or exit, fd: %d.\n", 
					ztime(), command_fd);
		...
		if(fd == data_fd[n])
		  {
			if(NULL != fp_connect_log)
			{
				fprintf(fp_connect_log, "ccd_image_gather: notify_server_eof: at %s: data connection for framegrabber %d CLOSED on error or exit, fd: %d.\n", 
						ztime(), n, data_fd[n]);
				fprintf(fp_connect_log, "Errno was %d (==> %s) (0 usually means EOF == client exited).\n\n", received_errno, strerror(received_errno));
				fflush(fp_connect_log);
			}
	...
	while(1)
	{
		...
		if(-1 != (command_fd = check_for_connection(command_s)))
		  {
			...
			if(NULL != fp_connect_log)
			{
				fprintf(fp_connect_log, "ccd_image_gather: at %s: command connection OPENED by command and control client, fd: %d.\n\n", 
						ztime(), command_fd);
				fflush(fp_connect_log);
			}
		  }
		...
		if(data_fd[q_blocks[n][m]] == -1)
			...
			if(-1 != (fd = connect_to_host_api(&data_fd[q_blocks[n][m]],q_hostnames[n],q_dports[n],NULL)))
			{
				...
				if(NULL != fp_connect_log)
				{
					fprintf(fp_connect_log, "ccd_image_gather: at %s: connection for data accepted host: %s port: %d (fd: %d)\n\n", ztime(),
								q_hostnames[n],q_dports[n],data_fd[q_blocks[n][m]]);
					fflush(fp_connect_log);
				}
			}
