...
int	main(int argc, char **argv)
  {
	...
	if(NULL != (cp = getenv("CCD_IMAGE_GATHER_CONNECTION_LOG")))
	{
		if(NULL == (fp_connect_log = fopen(cp, "a")))
		{
			fprintf(stderr, "WARNING:>>>\n\nConnection history file %s could not be created or opened.\n", cp);
			fprintf(stderr, "image_gather will allow operation, but this problem should be corrected.\n");
			fprintf(stderr, "\n<<<WARNING\n\n");
		}
		else
		{
			fprintf(fp_connect_log, "\n=========================================================================\n");
			fprintf(fp_connect_log, "\n\tccd_image_gather started at %s\n\n", ztime());
			fprintf(fp_connect_log, "\n=========================================================================\n");
		}
	}
