#ifndef DETCON_PAR_H_INCLUDE
#define DETCON_PAR_H_INCLUDE
 ...
 *-----------------------------------------------------------
 *
 *	Parameter lists for the set hardware and file parameter calls
 *	in the detcon_lib_th library.  These are indentical with the
 *	ADSC ESRF TACO device library parameters.
 *
 *-----------------------------------------------------------
 *
 */
...
enum {
	...
	HWP_MULT_TRIGTYPE,	/* multi-image trigger type, -1 = not defined (not a multi-frame sequence), 0 = sw trigger, 1 hw trigger */
	HWP_MULT_FRAME_START,	/* multi-image first frame number >= 0 */
	HWP_MULT_NFRAME,	/* multi-image set number of frames to collect >= 1 */
	HWP_MULT_SETTRIGGER	/* multi-image, 1 = sw trigger readout sequence; initial value 0; reset to zero after set to 1 */
     };
